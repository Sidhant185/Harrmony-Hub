...
export function PlayerControls({ isExpanded = false }: PlayerControlsProps) {
  ...
  return (
    <div
      className={`py-2 sm:py-3 ${isExpanded ? "pb-4" : ""}`}
      ...
      >
        ...
        <div
          className={`flex items-center gap-2 sm:gap-3 min-w-0 ${
            isExpanded ? "w-full justify-center" : "flex-1"
          }`}
        >
          {currentSong.coverArt ? (
            <div
              className={`relative ${
                isExpanded ? "w-24 h-24" : "w-12 h-12 sm:w-14 sm:h-14"
            ...
            <div
              className={`${
                isExpanded ? "w-24 h-24" : "w-12 h-12 sm:w-14 sm:h-14"
              } bg-primary/20 rounded-md flex items-center justify-center flex-shrink-0`}
            >
              <span className={isExpanded ? "text-4xl" : "text-xl sm:text-2xl"}>
                🎵
              </span>
            </div>
          )}
          <div className={`min-w-0 ${isExpanded ? "text-center" : "flex-1"}`}>
            <p
              className={`font-medium truncate text-white ${
                isExpanded ? "text-lg" : "text-xs sm:text-sm"
              } ${!isExpanded ? "max-w-[100px] sm:max-w-none" : ""}`}
            >
              {currentSong.title}
            </p>
            <p
        ...
        >
          ...
          >
            ...
            <button
              ...
              } bg-white text-black rounded-full hover:scale-110 transition-all shadow-lg`}
            >
              {isPlaying ? (
                <Pause
                  className={isExpanded ? "h-6 w-6" : "h-4 w-4 sm:h-5 sm:w-5"}
                />
              ) : (
                <Play
                  className={`${
          ...
          >
            <span
              className={`text-white/70 ${
                isExpanded
                  ? "text-sm w-12"
                  : "text-[10px] sm:text-xs w-8 sm:w-10"
              }`}
            >
        ...
        <div
          className={`flex items-center gap-1 sm:gap-2 min-w-0 ${
            isExpanded ? "w-full justify-center mt-2" : "flex-1 justify-end"
          }`}
        >
          <button
            onClick={() => {
